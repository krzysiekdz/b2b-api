...
class Controller_Admin_Eurofast_Orders extends Controller
{
    private $view;
    public $messageStr;
    ...
    public function before()
    {
        ...
        $this->link = $this->root . 'eurofast/orders';
    ...
    function fnc_OrderCell($table,$row,$name,$value)
    {
        if ($name=='nstatus')
        {
            ...
            if ($row['corrected']!='' && $v<5)
            {
                if ($row['corrected_type']==0) $value.='<br><span style="color:red;">ZMIANA ADRESU<br>'.$row['corrected'].'</span>';
                if ($row['corrected_type']==1) $value.='<br><span style="color:red;">ZMIANA KIEROWCY<br>'.$row['corrected'].'</span>';
                if ($row['corrected_type']==2) $value.='<br><span style="color:red;">ZMIANA AUTA<br>'.$row['corrected'].'</span>';
                if ($row['corrected_type']==3) $value.='<br><span style="color:red;">ZMIANA BAZY CELNEJ<br>'.$row['corrected'].'</span>';
                if ($row['corrected_type']==4) $value.='<br><span style="color:red;">ZMIANA BAZY DOSTAWCZEJ<br>'.$row['corrected'].'</span>';
                if ($row['corrected_type']==5) $value.='<br><span style="color:red;">ZMIANA NACZEPY<br>'.$row['corrected'].'</span>';
            ...
            $value.='</select>';
                ...
                $value='';
                $rr=sql_rows('SELECT id, kname, kstreet, kpostcode, kcity, kcountry, ndistance FROM eurofast_orders_pr WHERE iddoc=:id AND pquantity>0',array(':id'=>$row['id']));
                foreach ($rr as $rrr) {
                    if (trim($rrr['kname'])!='') {
                        $value.='<b>'.$rrr['kname'].'</b><br>'.$rrr['kstreet'].', '.$rrr['kpostcode'].' '.$rrr['kcity'].', '.$rrr['kcountry'].'<br>';
                    }
                }
    ...
    public function action_orders()
    {
        ...
        $t->where = 'nstatus>1 AND iduser=0';
        $t->limit=25;
        $t->leftJoin=array('eurofast_bazy#eurofast_bazy.id=eurofast_orders.idbase#eurofast_bazy.pname AS bpname',
                           'eurofast_cla#eurofast_cla.id=eurofast_orders.idclo#eurofast_cla.pname AS cpname',
                           'bs_users#bs_users.id=eurofast_orders.idspedycja#bs_users.pname AS upname',
            );
