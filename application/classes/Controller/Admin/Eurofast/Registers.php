...
/* **********************************************************************************
  Zarządzanie zarejestrowanymi odbiorcami
************************************************************************************/

class Controller_Admin_Eurofast_Registers extends Controller {
    private $view;
    public $messageStr;
    ...
    public function before() {
        ...
        $this->link=$this->root.'eurofast/registers';
        $this->view->sidebar_active='home';
        $this->view->sidebar_active_menu='registers';
        $this->table='eurofast_registers';


        $this->view->path=array(
            array('caption'=>'Home','url'=>$this->root),
            array('caption'=>'Zarejestrowani odbiorcy','url'=>$this->link),
        );
    ...
    {
        $this->view->title='Zarejestrowni odbiorcy';
        ...
        $t->tableView->title='Zarejestrowani odbiorcy';
        ...
        $t->options['toolButtons'] = array(array('caption' => 'Nowy odbiorca', 'href' => '/admin/eurofast/registers/edit'));
    ...
    public function action_edit()
    {
        ...
        if ($id<=0) {
            $this->view->path[] = array('caption' => 'Nowa odbiorca', 'url' => $this->link . '/edit');
            $this->view->title = 'Nowy odbiorca';
        } else {
            $this->view->path[] = array('caption' => 'Odbiorca', 'url' => $this->link . '/edit/'.$id);
            $this->view->title = 'Odbiorca';
        }

        $system = Model_BSX_System::init();

        $fields = array('pname','pcode','pnip','paddress','pbranche','ptaxwar');
        ...
        if ($id > 0) {
            ...
            //nowe
            $f['pname'] = $f['pcode'] = $f['pnip'] = $f['paddress'] = $f['ptaxwar'] = $f['pbranche'] ='';
            ...
            if ($save) {
                ...
                $this->messageStr = 'Nowy odbiorca został zapisany.';
            }
        }

        $view = Model_BSX_Core::create_view($this, 'Eurofast/part_form_admin_registers_edit', 'padmin');
        $view->set('f', $f);
        $view->set('id', $id);
        echo $view;
