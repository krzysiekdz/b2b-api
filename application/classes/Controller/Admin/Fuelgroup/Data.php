...
class Controller_Admin_Fuelgroup_Data extends Controller
{
    private $view;
    public $messageStr;
    ...
    public function before()
    {
        ...
        $this->link = $this->root . 'fuelgroup/cron';
        $this->view->sidebar_active = 'fuel';
        $this->view->sidebar_active_menu = 'cron';


        $this->view->path = array(
            array('caption' => 'Home', 'url' => $this->root),
            array('caption' => 'Cron', 'url' => $this->link),
        );
    ...
    public function action_list()
    {
        ...
        $t=new Model_BSX_Table('eurofast_orlen','tbl',$this);
        ...
        $t->showDelBtn=false;
        $t->showShowBtn=false;
        $t->options['toolbarVisible']=false;
    ...
    public function orlen_import()
    {
        ...
        sql_insert('eurofast_orlen',$dane);
    }

    public function action_cron()
    {
        //CRON ustawić na Godzina:55
        echo 'Cron v1.1 - '.date('Y-m-d H:i:s')."\n";
        ...
        Model_Fuelgroup_Core::createNotifications();


        //akceptacja za resellera - zamówienia na "dzisiaj"
        //$rows=sql_rows('SELECT id FROM eurofast_orders WHERE nstatus=1 AND ndate_term=:data AND modyf_time<:dt',array(':data'=>date('Y-m-d'),':dt'=>date('Y-m-d H:i:s',time()-20*60)));
        //foreach ($rows as $row)
        //{
        //    echo "Zmieniam status zamówienia $row[id] na ZAAKCEPTOWANY...";
        //    Model_Fuelgroup_Core::setStatus($row['id'],2);
        //    echo "OK!\n";
       // }


        //po wypełnieniu przez spedytora danych - zmiana statusu na "do wysłania"
        $rows=sql_rows('SELECT id FROM eurofast_orders WHERE nstatus=2 AND idbase>0 AND idclo>0 AND idspedycja>0 AND idkierowca>0 AND idauto>0 AND  ndate_term=:data AND modyf_time<:dt',array(':data'=>date('Y-m-d'),':dt'=>date('Y-m-d H:i:s',time()-20*60)));
        foreach ($rows as $row)
        {
            echo "Zmieniam status zamówienia $row[id] na DO REALIZACJI...";
            Model_Fuelgroup_Core::setStatus($row['id'],3);
            echo "OK!\n";
        }
        /*
                ...
                foreach ($rows as $row)
                {
                    ...
                    Model_Fuelgroup_Core::setStatus($row['id'],5);
                    echo "OK!\n";
                }
        */
        exit;
