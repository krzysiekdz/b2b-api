...
/* **********************************************************************************
  Kontrolery podstawowe - obsługujące różne funkcje panelu użytkownika, m.in.:
  - logowanie na konto użytkownika
  - wylogowywanie
************************************************************************************/

class Controller_Account_Core extends BSXController {

    private $statusOrdersList=array(0=>'W trakcie edycji',
                                    1=>'W trakcie realizacji',
                                    11=>'Oczekiwanie na dostawę',
                                    12=>'W trakcie kompletowania',
                                    13=>'Oczekiwanie na płatność',
                                    14=>'Gotowe do wysłania',
    ...
    public function before() {
        ...
        $this->view=Model_BSX_Core::create_view($this,'index_account','pshop');
        if (!$this->view) die('Nie załadowano szablonu index_account!');

        $this->root='account/';
        $this->link=$this->root;
        Model_BSX_Page::$active_tab='user';

        $this->view->path=array(
            array('caption'=>'Home','url'=>'/'),
            array('caption'=>'Konto','url'=>$this->root),
        );
    }
    ...
    public function action_detect()
    {
        ...
        if (!empty(Model_BSX_Core::$bsx_cfg['model']) && method_exists(Model_BSX_Core::$bsx_cfg['model'], 'actionDetect') && is_callable(array(Model_BSX_Core::$bsx_cfg['model'], 'actionDetect'),true))
        {
            $r=forward_static_call(array(Model_BSX_Core::$bsx_cfg['model'],'actionDetect'),$this,$w);
            if ($r!=false) return $r;
        }

        if ($w[0]=='login') return $this->showLogin($w);//logowanie uzytkownika
        else if ($w[0]=='logout') return $this->showLogout($w);
        else if ($w[0]=='home') return $this->showHome($w);
        else if ($w[0]=='profile') return $this->showProfile($w);
        else if ($w[0]=='company') return $this->showCompany($w);
        else if ($w[0]=='activate') return $this->showActivate($w);
        else if ($w[0]=='orders') return $this->showOrders($w);
        else if ($w[0]=='order') return $this->showOrder($w);
        else if ($w[0]=='remind') return $this->showRemind($w);
        else if ($w[0]=='register') return $this->showRegister($w);
        ...
        else if ($w[0]=='users_list') return $this->showCompanyEmployees($w);//uzytkownicy przypisani do danego oddzialu
        else if ($w[0]=='companies_list') return $this->showCompanies($w);//wszystkie oddzialy
    ...
    private $b2b_statuses_a=array(1,10,20,30);
    private $b2b_statuses_abr=array(10,20,30);
    private $err='';
    private $err_code=0;
    private $fields=array();
    // /create_update_user
    private function showCreateUpdateUser($w) // tworzenie i modyfikacja uzytkownika  
    {
