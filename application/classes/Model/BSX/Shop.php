...
class Model_BSX_Shop
{
    ...
    {
        if (empty($row['apname'])) {
            $at=sql_row('SELECT id, pid, ptable, pname FROM bs_attachments WHERE ptable="bs_stockindex" AND pid=:id AND pstatus=2 ORDER BY prating DESC',array(':id'=>$row['id']));
            if ($at) {
                sql_query('UPDATE bs_stockindex SET mimageid=:newid WHERE id=:id',array(':newid'=>$at['id'],':id'=>$row['id']));
                $row['apname']=$at['pname'];
                $row['pid']=$at['pid'];
                $row['ptable']=$at['ptable'];
            }
            //echo '<pre>';
            //print_r($at);
            //die('x');
    ...
    public function getCategories($idr=0) {
        ...
        if (count($lista)<=0) {
            ...
            foreach ($rows as $row) {
                $lista[$row['idparent']][] = $row;//jesli nie ma rodzica to zapisuje sie w pustym polu czyli w takim: []
